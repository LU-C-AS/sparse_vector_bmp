...
//     static void GetSizeToPtr(char *&p, const Vector<const BlockData<DataType> *> &block_data_list) {
...
//     static void WriteToPtr(char *&p, const Vector<const BlockData<DataType> *> &block_data_list) {
//         SizeT num_sum = 0;
...
public:
    ...
    BlockPostingsBase(i32 kth, DataType kth_score, BlockData<DataType> data)
        : kth_(kth), kth_score_(kth_score), data_(std::move(data)) {}

    DataType kth(i32 topk) const { return topk == kth_ ? kth_score_ : 0.0; }

    const BlockData<DataType> &data() const { return data_; }
    BlockData<DataType> &data() { return data_; }
...
template <typename DataType>
struct BlockPostings : public BlockPostingsBase<DataType> {
    // using BlockData = BlockData<DataType>;

    // BlockPostings(i32 kth, DataType kth_score, BlockData data)
    //     : BlockPostingsBase<DataType>(kth, kth_score, std::move(data)) {}

public:
    BlockPostings() = default;

    BlockPostings(i32 kth, DataType kth_score, BlockData<DataType> data)
        : BlockPostingsBase<DataType>(kth, kth_score, std::move(data)) {}

    SizeT GetSizeInBytes() const { return sizeof(this->kth_) + sizeof(this->kth_score_) + this->data_.GetSizeInBytes(); }

    static Tuple<SizeT, const i32 *, const DataType *> ReadFromPtrToTuple(const char *&p) {
    ...
    static BlockPostings ReadAdv(const char *&p) {
        ...
        res.data_ = BlockData<DataType>::ReadAdv(p);
        return res;
    }
    ...
    static void GetSizeToPtr(char *&p, const Vector<BlockPostings> &postings) {
        ...
        Vector<const BlockData<DataType> *> block_data_list;
        for (const auto &posting : postings) {
            block_data_list.push_back(&posting.data_);
        }
        BlockData<DataType>::GetSizeToPtr(p, block_data_list);
    }
    ...
    static void WriteToPtr(char *&p, const Vector<BlockPostings> &postings) {
        ...
        BlockData<DataType>::WriteToPtr(p, block_data_list);
    }

    static Vector<BlockPostings> ReadFromPtrToVector(const char *&p) {
        Vector<BlockPostings> res;
        SizeT posting_num = ReadBufAdvAligned<SizeT>(p);
        const i32 *kths = ReadBufVecAdvAligned<i32>(p, posting_num);
        const DataType *kth_scores = ReadBufVecAdvAligned<DataType>(p, posting_num);
        Vector<BlockData<DataType>> block_data_list = BlockData<DataType>::ReadFromPtr(p, posting_num);
        for (SizeT i = 0; i < posting_num; ++i) {
...
//     BlockPostings(i32 kth, DataType kth_score, BlockData<DataType, CompressType> data)
//         : BlockPostingsBase<DataType, CompressType>(kth, kth_score, std::move(data)) {}
// };
...
class BMPIvt {
private:
    // using BlockPostings = BlockPostings<DataType>;

    BMPIvt(Vector<BlockPostings<DataType>> postings) : postings_(std::move(postings)) {}
...
public:
    ...
    const BlockPostings<DataType> &GetPostings(SizeT term_id) const { return postings_[term_id]; }
    ...
    static BMPIvt ReadAdv(const char *&p) {
        ...
        Vector<BlockPostings<DataType>> postings(posting_size);
        for (SizeT i = 0; i < posting_size; ++i) {
            postings[i] = BlockPostings<DataType>::ReadAdv(p);
        }
        return BMPIvt(std::move(postings));
    }

    void GetSizeToPtr(char *&p) const { BlockPostings<DataType>::GetSizeToPtr(p, postings_); }

    void WriteToPtr(char *&p) const { BlockPostings<DataType>::WriteToPtr(p, postings_); }

    static BMPIvt ReadFromPtr(const char *&p) { return BMPIvt(BlockPostings<DataType>::ReadFromPtr(p)); }

private:
    Vector<BlockPostings<DataType>> postings_;
protected:
